...
export default function LecDetailsPage() {
  ...
  const EmptyVideosState = () => (
    <div className="text-center py-12">
      ...
      <h3 className="text-lg font-medium text-foreground mb-2">لا توجد فيديوهات</h3>
      <p className="text-sm text-muted-foreground mb-4">
        لم يتم إضافة أي فيديوهات لهذه المحاضرة بعد
      </p>
        <VideoForm />
    </div>
  );
  ...
  const EmptyFilesState = () => (
    <div className="text-center py-12">
      ...
      <p className="text-sm text-muted-foreground mb-4">
        لم يتم إضافة أي ملفات لهذه المحاضرة بعد
      </p>
  ...
  const EmptyQuizzesState = () => (
    <div className="text-center py-12">
      ...
      <h3 className="text-lg font-medium text-foreground mb-2">لا توجد اختبارات</h3>
      <p className="text-sm text-muted-foreground mb-4">
        لم يتم إضافة أي اختبارات لهذه المحاضرة بعد
      </p>
  ...
  const QuestionEditDialog = ({ question, quizId }) => {
    ...
    const addAnswer = () => {
      if (watchQuestionType === "tof") return;
  ...
  const AnswerEditDialog = ({ answer, questionId }) => {
    ...
    const { mutate: updateAnswer } = useUpdate({ 
      service: (id, body) => apiRequest("patch", 
